...
class PDITLClassifier(PDIClassifier):
    ...
    def __init__(self, n_classes, tl_tag='mobilenet', img_size=70, pdi_strategy='tsne',
        ...
        if tl_tag == 'vgg':
            tl_model = VGG16(include_top=False, weights='imagenet', input_shape=(128, 128, 3))
        elif tl_tag == 'resnet':
            tl_model = ResNet50V2(include_top=False, weights='imagenet', input_shape=(128, 128, 3))
        else:
            tl_model = MobileNet(include_top=False, weights='imagenet', input_shape=(128, 128, 3))
